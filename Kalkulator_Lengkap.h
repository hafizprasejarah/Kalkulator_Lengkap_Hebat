...
private: System::Void equalButton_Click(System::Object^ sender, System::EventArgs^ e) {
    ...
    if (parts[0] == "Tan" || parts[0] == "Cos" || parts[0] == "Sin" && parts->Length <= 2) {
        if (parts[0] == "Sin")
        {
            double radian = Double::Parse(parts[1]) * M_PI / 180.0; //Rumus radian
            double hasilSin = sin(radian);
            displayBox->Text = hasilSin.ToString();

            return;
        }
        else if (parts[0] == "Cos") {
            double radian = Double::Parse(parts[1]) * M_PI / 180.0;
            double hasilSin = cos(radian);
            displayBox->Text = hasilSin.ToString();
            ...
            double radian = Double::Parse(parts[1]) * M_PI / 180.0;
            double hasilSin = tan(radian);
            displayBox->Text = hasilSin.ToString();

            return;
        }
    }
    else if (parts[0] == "Tan" || parts[0] == "Cos" || parts[0] == "Sin" && parts->Length >= 2) {
        displayBox->Text = "Salah Ungkapan";
        return;
    }
